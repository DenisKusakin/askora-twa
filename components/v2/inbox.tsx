...
function InboxItem({addr, amount, createdAt, className, id, isClosed, isRejected}: {
    addr: Address,
    owner: Address,
    createdAt: number,
    amount: bigint,
        ...
        <div className={"flex flex-row justify-between w-full"}>
            ...
            <div className={"flex flex-row w-3/12 justify-center"}>
                <div className={"flex flex-row justify-center items-center"}>
                    <div className={`text-lg text-right ${additional_class}`}>{parseFloat(fromNano(amount)).toFixed(3)}</div>
...
export default function MyInbox() {
    ...
    return <div className={"pt-10"}>
        ...
        <div className={"flex w-full mt-4 flex-col"}>
            ...
            {!assigned?.isLoading && assigned?.data?.map(x => <InboxItem key={x.addr.toString()} addr={x.from}
                                                                         id={x.id}
                                                                         owner={x.to}
                                                                         isClosed={x.isClosed}
